...
React Native / Expo (SDK 57) app for a book collection. Expo Router, NativeWind, Supabase,
...
**Start at `STATUS.md`.** It is the handover: what is done, what is left, in order, plus
the traps. Read it before planning anything.

**Lidar, Radar (`../radar`) and Sonar (`../sonar`) share one Supabase project.** Accounts,
profiles, friendships, the privacy setting and the theme preference are the same rows in
all three apps. Read `docs/shared-database.md` before touching anything under `supabase/`
or any table Radar owns.
...
- All reads go through `lib/normalizeBook`; all writes through `toBookRow` +
  `stripUndefined`.
...
- Conventional Commits: `feat(library): group the shelf by series`. Subject in imperative
  mood, ~50 chars, no trailing period. Body only when the "why" is not obvious from the
  subject.
...
- Present tense, sentence case, no trailing period, ~90 chars max. Say what the user can
  now do, and name the surface (Library, Ratings, Stats, Social, Profile, Settings, book
  page, the scanner, History).
...
- New pure logic in `src/lib/` or a feature hook gets a co-located `*.test.ts`. Existing
  pattern: `src/lib/isbn.test.ts`, `src/lib/bookKey.test.ts`, `src/lib/stats.test.ts`.
...
- The Firebase project id lives in `.firebaserc` (gitignored). If that file is missing,
  the project exists but has never been linked here — ask the user for the id rather than
  guessing, then `firebase use --add`. Hosting serves `dist/` with an SPA rewrite to
  `/index.html` (`firebase.json`). `dist/` is gitignored — never commit build output.
...
2. `app.json`: `expo.version` matches, `versionCode` bumped; the About version in
...
7. Update `STATUS.md` so the next session starts from the truth.
...
- Radar's `supabase/schema.sql` must have been run first — Lidar's file starts with a
...
- Never write a table Radar or Sonar owns except through the two shared `user_settings`
