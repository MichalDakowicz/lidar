...
export function useActivityComments(activityId: string | undefined) {
  ...
  const queryKey = useMemo(() => ['bookComments', activityId] as const, [activityId]);

  const query = useQuery({
    queryKey,
    queryFn: async () => {
      const { data, error } = await supabase
        .from('book_activity_comments')
        .select('*')
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`book_comments:${activityId}:${Math.random().toString(36).slice(2)}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'book_activity_comments', filter: `activity_id=eq.${activityId}` },
  ...
  const remove = useMutation({
    mutationFn: async (commentId: string) => {
      const { error } = await supabase.from('book_activity_comments').delete().eq('id', commentId);
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey }),
  });
...
export function useCommentCounts(activityIds: string[]) {
  ...
  const query = useQuery({
    queryKey: ['bookCommentCounts', ids.join(',')],
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('book_activity_comments').select('activity_id').in('activity_id', ids);
      if (error) throw error;
