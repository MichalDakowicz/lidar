...
type FeedViewProps = {
  me: Profile | null;
  friends: Profile[];
  events: BookActivityEvent[];
  loading: boolean;
  /** Watermark of your last visit — what "new" is counted against. */
  since: string | null;
  /** Cover lookup by book key, from your own library and your friends'. */
  coverFor: (bookKey: string | null) => string | null;
...
export function FeedView({ me, friends, events, loading, since, coverFor, selfId }: FeedViewProps) {
  ...
  const actors = useProfileMap(useMemo(() => events.map((event) => event.userId), [events]));
        ...
        {visible.length === 0 ? (
          ...
          visible.map((event) => (
            <FeedCard
              key={event.id}
              event={event}
              actor={actors.get(event.userId)}
              coverUrl={coverFor(event.bookKey)}
              ...
              onOpenBook={() =>
                event.bookKey
                  ? router.push({ pathname: '/release/[bookKey]', params: { bookKey: event.bookKey } })
                  : undefined
              }
