...
export function useActivityReactions(activityIds: string[]) {
  ...
  const queryKey = useMemo(() => ['bookReactions', ids.join(',')] as const, [ids]);

  const query = useQuery({
    queryKey,
    queryFn: async () => {
      const { data, error } = await supabase.from('book_activity_reactions').select('*').in('activity_id', ids);
