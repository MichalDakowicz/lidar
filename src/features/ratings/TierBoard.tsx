 ...
 * books in A, one in D) keeps every row the same height and the letters stay
...
function Row({ row, onPick, onSearch }: { row: TierRow; onPick: (rating: BookRating) => void; onSearch?: () => void }) {
    ...
    >
      ...
      {ratings.length === 0 ? (
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="items-center gap-2 p-2">
          {ratings.map((rating) => (
            <Cover key={rating.bookKey} rating={rating} onPress={() => onPick(rating)} />
          ))}
        </ScrollView>
      )}
    </View>
...
function Cover({ rating, onPress }: { rating: BookRating; onPress: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} className="active:opacity-70" style={{ width: COVER }}>
      ...
      <Text numberOfLines={1} className="text-[10px] text-muted-foreground">
        {authorsToDisplayString(rating.authors)}
      </Text>
    </Pressable>
  );
}
