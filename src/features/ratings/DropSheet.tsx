 ...
 * rating a stack of books is six visible buttons rather than six sliders. The
...
export const DropSheet = forwardRef<BottomSheetModal, DropSheetProps>(function DropSheet(
  ...
  const existing = target ? ratingFor(target.bookKey) : null;
          ...
          <>
            <View className="flex-row items-center gap-3">
              ...
              <View className="min-w-0 flex-1">
                ...
                <Text numberOfLines={1} className="text-xs text-muted-foreground">
                  {[authorsToDisplayString(target.authors ?? []), publishedYear(target.publishedDate)].filter(Boolean).join(' • ')}
                </Text>
            ...
            <View className="gap-2 border-t border-border pt-4">
              <Pressable
                onPress={() => {
                  dismiss();
                  router.push({ pathname: '/release/[bookKey]', params: { bookKey: target.bookKey } });
                }}
