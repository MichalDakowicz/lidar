...
// the moment a freshly added book lands in the row.
type BookCarouselProps = {
  title?: string;
  badge?: string;
  books: Book[];
  cardVariant?: BookCardVariant;
  cardWidth?: number;
      ...
      <View className="relative" onLayout={onLayout}>
        ...
        >
          {books.map((book, index) => (
            <View key={book.id} style={{ width: itemWidth, marginRight: index === books.length - 1 ? 0 : GAP }}>
              <BookCard
                book={book}
                variant={cardVariant}
