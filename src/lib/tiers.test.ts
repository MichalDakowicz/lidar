...
function rating(bookKey: string, ratings: Ratings, review = ''): BookRating {
  return {
    userId: 'u',
    bookKey,
    isbn13: null,
...
describe('tierFor', () => {
  ...
  it('is null for no score, so an unrated book is on no row', () => {
...
describe('groupByTier', () => {
  ...
  it('files each rating under its band', () => {
    ...
    expect(rows[0].ratings.map((entry) => entry.bookKey)).toEqual(['top']);
    expect(rows[2].ratings.map((entry) => entry.bookKey)).toEqual(['mid']);
  });

  it('reads a facet-only rating through the same score rule', () => {
    // No overall, facets average to 4.5 → S.
    const rows = groupByTier([rating('facets', { prose: 5, plot: 4 })]);
    expect(rows[0].ratings.map((entry) => entry.bookKey)).toEqual(['facets']);
  });
  ...
  it('orders a row highest first, then by title', () => {
    ...
    expect(rows[1].ratings.map((entry) => entry.bookKey)).toEqual(['best', 'a', 'b']);
  });
});
...
describe('withReviews', () => {
  it('keeps only ratings that carry written thoughts', () => {
    ...
    expect(kept.map((entry) => entry.bookKey)).toEqual(['a']);
  });
});
