 ...
 * second copy of the same fact, free to disagree with it. Dropping an book into
...
export type Tier = {
  ...
  min: number;
  /** What dropping an book here sets the overall to. */
  score: number;
  color: string;
  tint: string;
};
...
export type TierRow = { tier: Tier; ratings: BookRating[] };
 ...
 */
export function groupByTier(ratings: BookRating[]): TierRow[] {
  const buckets = new Map<TierId, BookRating[]>(TIERS.map((tier) => [tier.id, []]));
...
export function withReviews(ratings: BookRating[]): BookRating[] {
