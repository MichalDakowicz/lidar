...
// Time-period scoping for the Stats screen. Lidar's stats have two halves and
// the period only touches one of them: what you *played* is an event with a
// date, so it scopes; what you *own* is a shelf, and narrowing a library by
...
// trimmed read log, and the stat builders run unchanged on top.
...
export function scopeReadsToPeriod(reads: Read[], start: Date | null): Read[] {
  if (!start) return reads;
  const from = start.getTime();
  return reads.filter((read) => {
    const at = new Date(read.finishedAt).getTime();
