...
//
// The feed reads public.book_activity rows written by useBooks' logActivity,
...
/**
 * Reaction codes match the CHECK constraint on book_activity_reactions.kind.
...
type MinimalEvent = { type: BookActivityType; details: Record<string, unknown> | null };
...
export function weekDigest(
  events: Pick<BookActivityEvent, 'userId' | 'createdAt' | 'type' | 'details'>[],
  now: number = Date.now(),
  windowDays = 7,
  maxBars = 4,
): WeekDigest {
...
export function freshCountsSince(
  events: Pick<BookActivityEvent, 'userId' | 'createdAt'>[],
