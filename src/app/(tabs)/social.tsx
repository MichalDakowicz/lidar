...
function SocialScreen() {
  ...
  // survive the book being deleted), so artwork is looked up by release key
  // from what this device already has cached: your shelf and your ratings.
  const coverByKey = useMemo(() => {
    const map = new Map<string, string>();
    for (const rating of ratings) if (rating.coverUrl) map.set(rating.bookKey, rating.coverUrl);
    for (const book of books) if (book.coverUrl) map.set(book.bookKey, book.coverUrl);
    return map;
  }, [books, ratings]);
      ...
      <ContentShell fill maxWidth={MAX_W.text}>
        {segment === 'activity' && (
          <FeedView
            ...
            since={since}
            selfId={user?.id}
            coverFor={(bookKey) => (bookKey ? coverByKey.get(bookKey) ?? null : null)}
          />
        )}
        {segment === 'friends' && (
          <FriendsView
